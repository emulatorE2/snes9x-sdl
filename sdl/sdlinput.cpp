...
void S9xParseInputConfig (ConfigFile &conf, int pass)
{
	...
	if (!conf.GetBool("Unix::ClearAllControls", false))
	{
		...
		keymaps.push_back(strpair_t("K00:SDLK_r",            "Joypad1 Select"));
		keymaps.push_back(strpair_t("K00:SDLK_y",            "Joypad1 Start"));
		//keymaps.push_back(strpair_t("K00:SDLK_z",            "Joypad1 ExitEmu"));
		//keymaps.push_back(strpair_t("K00:SDLK_x",            "Joypad1 Pause"));
		...
		case SDL_KEYUP:		
			if (event.key.keysym.sym == 57)
			{
				...
				if (event.key.keysym.sym == 48) {
					event.key.keysym.sym = (SDLKey)13;
				} else if (event.key.keysym.sym == 55) {
					event.key.keysym.sym = (SDLKey)99;
				}				
